...
class DoBusinessWithController extends Controller
{
    function list()
    {
        ...
        $do_business_with = Dobusinesswith_Model::join('video_categories', 'video_categories.id', '=', 'do_business_with.category_id')
            ->select('video_categories.name as category', 'do_business_with.*')->paginate(10);
        $category = VideoCategory::where('status', 'active')->get();
        return view('do_business_with.list', compact('do_business_with', 'category', 'categories'));
    }
    function create()
    {
        $categories = VideoCategory::where('status', 'active')->get();
